...
import { ArrowRight, BarChart3, ClipboardPenLine, Crosshair, Sparkles, TrendingDown, TrendingUp } from "lucide-react";
    ...
    <div className="mx-auto max-w-7xl pb-10">
      <section className="rise-in relative overflow-hidden rounded-2xl border border-[#dba71d]/45 bg-[#14150f] px-6 py-7 md:px-9 md:py-9">
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div><div className="mb-3 flex items-center gap-2 text-[#e2b540]"><Crosshair className="h-4 w-4" /><span className="text-[11px] font-bold uppercase tracking-[0.17em]">Bashfx VIP Gold Room / Multi-market M5</span></div><h1 className="font-display text-5xl tracking-wide text-white md:text-6xl">YOUR WEEK, IN FOCUS.</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-[#b5b2a5]">{isAuthenticated ? `Welcome back${user?.name ? `, ${user.name}` : ""}. Record each dated market trade, review your weekly ledger, then generate a polished Gold Room report.` : "Sign in to keep your dated multi-market journal and generated performance summaries private."}</p></div>
          <Link href="/upload"><Button className="h-12 bg-[#dba71d] px-5 font-bold text-[#171005] hover:bg-[#f3ce66]"><ClipboardPenLine className="mr-2 h-4 w-4" />Open daily journal</Button></Link>
        </div>
      </section>
      ...
      <section className="mt-7 grid gap-5 lg:grid-cols-[1.4fr_.8fr]"><div className="metal-border rounded-xl bg-[#12130e] p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-[#948e79]">The Gold Room workflow</p><h2 className="mt-2 font-display text-3xl tracking-wide text-white">FROM DATE TO SUMMARY</h2></div><BarChart3 className="h-7 w-7 text-[#dba71d]" /></div><div className="mt-7 grid gap-4 md:grid-cols-3"><Workflow number="01" title="Journal" body="Record every market trade with its own required date." icon={<ClipboardPenLine className="h-4 w-4" />} /><Workflow number="02" title="Review" body="Check the dated weekly ledger and improve your process notes." icon={<Crosshair className="h-4 w-4" />} /><Workflow number="03" title="Generate" body="Create a branded weekly summary image ready to export." icon={<Sparkles className="h-4 w-4" />} /></div></div><div className="metal-border relative overflow-hidden rounded-xl bg-[linear-gradient(155deg,#1a170c,#0f110d_55%,#152015)] p-6"><div className="absolute -right-5 -top-5 h-32 w-32 rounded-full bg-[#dba71d]/15 blur-2xl" /><div className="relative"><div className="flex items-center gap-2 text-[#e4b933]"><TrendingUp className="h-5 w-5" /><p className="text-[11px] font-bold uppercase tracking-[0.16em]">Ready when you are</p></div><h2 className="mt-4 font-display text-4xl tracking-wide text-white">MAKE THE WEEK COUNT.</h2><p className="mt-3 text-sm leading-6 text-[#b7b4a8]">Every individual trade date drives your weekly result. Generate the image after you have checked the ledger.</p><Link href="/summary"><Button variant="outline" className="mt-6 border-[#dba71d]/60 bg-transparent text-[#f3d26d] hover:bg-[#dba71d]/10 hover:text-[#ffe9a3]">Open summary studio <ArrowRight className="ml-2 h-4 w-4" /></Button></Link><TrendingDown className="absolute bottom-0 right-0 h-20 w-20 rotate-180 text-[#dba71d]/10" /></div></div></section>
    </div>
  );
}
