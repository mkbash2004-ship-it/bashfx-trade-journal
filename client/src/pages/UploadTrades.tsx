  ...
  return (
    <article id={cardId} className={`scroll-mt-6 overflow-hidden rounded-xl border bg-[#0d0e0a] ${hasMissingFields ? "border-[#b74e46] shadow-[0_0_0_1px_rgba(183,78,70,.18)]" : "border-[#594a1d]"}`}>
      ...
      <div className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Field label="Pips — signed & required" value={trade.pips} onChange={(value) => onPatch({ pips: value })} type="number" placeholder="+75" invalid={isMissing("pips")} suggestion={REQUIRED_TRADE_FIELD_SUGGESTIONS.pips} />
...
function Field({ label, value, onChange, placeholder, type = "text", invalid = false, suggestion }: { label: string; value: string; onChange: (value: string) => void; placeholder?: string; type?: string; invalid?: boolean; suggestion?: string }) {
  return <div><Label className={`text-[10px] font-bold uppercase tracking-[0.12em] ${invalid ? "text-[#ff9a91]" : "text-[#9f9b8e]"}`}>{label}</Label><Input aria-invalid={invalid} type={type} value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className={`mt-1.5 h-9 bg-[#090a07] text-sm text-white placeholder:text-[#5e5b51] ${invalid ? "border-[#cb6057] focus:border-[#ff9a91]" : "border-[#403a20] focus:border-[#dba71d]"}`} />{invalid && <p className="mt-1 text-[10px] leading-4 text-[#f99b92]">{suggestion ?? "Required before saving."}</p>}</div>;
}
